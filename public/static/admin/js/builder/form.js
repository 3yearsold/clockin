...
jQuery(document).ready(function() {
    ...
    $('.js-bmap').each(function () {
        ...
        var radius_input  = $self.find('.bmap-radius');
        ...
        var create_mark = function (point) {
            ...
            var radius = parseFloat(radius_input.val()) || 0;
            if (radius > 0) {
                circleOverlay = new BMap.Circle(point, radius, {
                    strokeColor: 'red',
                    strokeWeight: 1,
                    strokeOpacity: 0.3
                });
                map.addOverlay(circleOverlay);
                map.setViewport(circleOverlay.getBounds());
            }
            // 写入坐标
        ...
        radius_input.on('input', function() {
            var curr_point = map_point.val().split(',');
            if (curr_point.length == 2) {
                var point = new BMap.Point(curr_point[0], curr_point[1]);
                create_mark(point);
            }
        });
